...
public class InAppWebView: WKWebView, UIScrollViewDelegate, WKUIDelegate,
    ...
    // GOTOK FORK: src of the image the native context menu was opened on.
    var nativeImageContextMenuSrc: String?
    // GOTOK FORK: viewport CSS rect of that image, used to lift the image
    // itself when the element is a link (instead of WebKit's link preview).
    var nativeImageContextMenuCssRect: CGRect?
    ...
    func replaceGestureHandlerIfNeeded() {
        // GOTOK FORK: keep WebKit's native long-press / context-menu interaction
        // intact when the native image context menu is enabled, otherwise the
        // hijack below suppresses contextMenuConfigurationForElement.
        if settings?.nativeImageContextMenuEnabled == true {
            return
        }
    ...
    // GOTOK FORK: native Chrome-on-WKWebView style long-press context menu for
    // images. Gated by `nativeImageContextMenuEnabled` so other webviews keep
    // their default behavior. The image src is resolved via a JS hit-test at the
    // last touch point and selected actions are bridged to Dart through the
    // standard JS bridge: window.flutter_inappwebview.callHandler('onWebImageAction', ...).
    @available(iOS 13.0, *)
    public func webView(_ webView: WKWebView,
                        contextMenuConfigurationForElement elementInfo: WKContextMenuElementInfo,
                        completionHandler: @escaping (UIContextMenuConfiguration?) -> Void) {
        guard settings?.nativeImageContextMenuEnabled == true,
              configuration.preferences.javaScriptEnabled,
              let touchPoint = lastTouchPoint else {
            nativeImageContextMenuSrc = nil
            nativeImageContextMenuCssRect = nil
            completionHandler(nil)
            return
        }

        let x = touchPoint.x / scrollView.zoomScale
        let y = touchPoint.y / scrollView.zoomScale
        let js = """
        (function(){
          try {
            var el = document.elementFromPoint(\(x), \(y));
            var img = null, node = el;
            while (node) {
            ...
            if (!img) { return JSON.stringify({ src: null }); }
            var r = img.getBoundingClientRect();
            return JSON.stringify({ src: img.currentSrc || img.src,
                                    x: r.left, y: r.top, w: r.width, h: r.height });
          } catch (e) { return JSON.stringify({ src: null }); }
        })()
        """

        // When the pressed element is (or is inside) a link, WebKit's default
        // preview is the link preview card that loads the target page. That
        // hides the image the seller is picking, so we lift the image itself.
        let isLinkElement = elementInfo.linkURL != nil
        evaluateJavaScript(js) { [weak self] (value, _) in
            ...
            if let str = value as? String,
               ...
               let obj = (try? JSONSerialization.jsonObject(with: data)) as? [String: Any] {
                ...
                if let rx = obj["x"] as? Double, let ry = obj["y"] as? Double,
                   let rw = obj["w"] as? Double, let rh = obj["h"] as? Double,
                   rw > 0, rh > 0 {
                    cssRect = CGRect(x: rx, y: ry, width: rw, height: rh)
                }
            }
            guard let imageSrc = src, !imageSrc.isEmpty else {
                self.nativeImageContextMenuSrc = nil
                self.nativeImageContextMenuCssRect = nil
                completionHandler(nil)
                return
            }
            self.nativeImageContextMenuSrc = imageSrc
            self.nativeImageContextMenuCssRect = cssRect
            completionHandler(self.buildNativeImageContextMenuConfiguration(
                src: imageSrc,
                previewImageInsteadOfLink: isLinkElement))
        }
    }

    @available(iOS 13.0, *)
    private func buildNativeImageContextMenuConfiguration(src: String,
                                                          previewImageInsteadOfLink: Bool) -> UIContextMenuConfiguration {
        // This webview exists only to pick an image for the cutout / product
        // photo flow, so the menu is intentionally focused on that single goal.
        // The native lift already previews the image, and tapping it commits the
        // same "use" action (see willCommitWithAnimator).
        let actionProvider: UIContextMenuActionProvider = { [weak self] _ in
            let use = UIAction(title: NSLocalizedString("Use this photo", comment: ""),
                               image: UIImage(systemName: "checkmark.circle")) { _ in
                self?.sendWebImageAction("use", src: src)
            }
            return UIMenu(title: "", children: [use])
        }
        // Bare <img> elements keep WebKit's default image lift. Anchor-wrapped
        // images get a snapshot of the image rect so the link preview never
        // replaces the image highlight.
        var previewProvider: UIContextMenuContentPreviewProvider? = nil
        if previewImageInsteadOfLink, let cssRect = nativeImageContextMenuCssRect {
            previewProvider = { [weak self] in
                self?.buildNativeImagePreviewController(cssRect: cssRect)
            }
        }
    ...
    // GOTOK FORK: preview controller that shows a snapshot of just the image's
    // on-screen rect, mimicking the system image lift for anchor-wrapped images.
    @available(iOS 13.0, *)
    private func buildNativeImagePreviewController(cssRect: CGRect) -> UIViewController? {
        let zoom = scrollView.zoomScale
        let viewRect = CGRect(x: cssRect.origin.x * zoom,
                              y: cssRect.origin.y * zoom,
                              width: cssRect.width * zoom,
                              height: cssRect.height * zoom).intersection(bounds)
        guard !viewRect.isEmpty else { return nil }

        let controller = UIViewController()
        controller.view.backgroundColor = .systemBackground
        let imageView = UIImageView(frame: CGRect(origin: .zero, size: viewRect.size))
        imageView.contentMode = .scaleAspectFill
        imageView.clipsToBounds = true
        imageView.autoresizingMask = [.flexibleWidth, .flexibleHeight]
        controller.view.addSubview(imageView)

        // Cap the lifted card so full-width hero images don't cover the page.
    ...
    public func webView(_ webView: WKWebView,
        ...
        // Tapping the lifted preview itself uses the photo (Chrome-like commit).
        guard settings?.nativeImageContextMenuEnabled == true,
              let src = nativeImageContextMenuSrc else {
            return
        }
    ...
    // GOTOK FORK: bridge a native context-menu action back to Dart.
    private func sendWebImageAction(_ action: String, src: String) {
        let payload: [String: Any] = ["action": action, "src": src]
        ...
        let bridge = JavaScriptBridgeJS.get_JAVASCRIPT_BRIDGE_NAME()
        let source = "if (window.\(bridge) && window.\(bridge).callHandler) { window.\(bridge).callHandler('onWebImageAction', '\(escaped)'); }"
